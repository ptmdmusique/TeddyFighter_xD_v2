...
public class StatusIndicator : MonoBehaviour {
	...
	[Header("Text Effect")]
	public float maxTextValue = -1;
	public bool useTextLerp = true;
	public float targetTextVal = 0;
	private float curTextVal = 0;
    ...
    private void Update() {
		...
		curTextVal = useTextLerp == true && ((curTextVal < targetTextVal && curTextVal < targetTextVal * 0.9f) || (curTextVal > targetTextVal && curTextVal > targetTextVal * 0.9f)) ?
	Mathf.Round(Mathf.Lerp(curTextVal, targetTextVal, Time.deltaTime * lerpSpeed) * 100) / 100 : targetTextVal;
		if (statusText != null) {
			statusText.text = curTextVal + "/" + maxTextValue + " " + statusName;
		}
    ...
    public void SetValue(float curValue, float maxValue) {
        ...
        if (mapAlpha == true) {
            ...
            if (inverseAlpha == false) {
                float newValue = StaticGlobal.Map(0, maxValue, 0, 1, curValue * alphaMultiplier);
                targetAlpha.a = Mathf.Lerp(targetAlpha.a, newValue, Time.deltaTime * lerpSpeed * 10);
                if (curValue == 0) {
                    targetAlpha.a = 0;
                }
            } else {
                float newValue = StaticGlobal.Map(0, maxValue, 0, 1, (maxValue - curValue) * alphaMultiplier);
                targetAlpha.a = Mathf.Lerp(targetAlpha.a, newValue, Time.deltaTime * lerpSpeed * 10);
                if (curValue == 0) {
        ...
        if (mapColor == true) {
            float newValue = StaticGlobal.Map(0, maxValue, 0, 1, curValue);
